...
export const poolToV3SubgraphPool = (pool: Pool, idx: number): V3SubgraphPool => {
	return {
		id: idx.toString(),
...
export const pairToV2SubgraphPool = (pool: Pair, idx: number): V2SubgraphPool => {
	return {
		id: idx.toString(),
		token0: {
			id: pool.token0.address,
...
export const buildMockV3PoolAccessor: (pools: Pool[]) => V3PoolAccessor = (pools: Pool[]) => {
	return {
		...
		getPoolByAddress: (address: string) =>
			_.find(
				pools,
				(p) => Pool.getAddress(p.token0, p.token1, p.fee).toLowerCase() == address.toLowerCase()
			),
		getPool: (tokenA, tokenB, fee) =>
			_.find(
				pools,
				(p) => Pool.getAddress(p.token0, p.token1, p.fee) == Pool.getAddress(tokenA, tokenB, fee)
			),
	};
};
