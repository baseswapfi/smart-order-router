...
describe('alpha router', () => {
	...
	beforeEach(() => {
		...
		const mockTokens = [USDC, DAI, WRAPPED_NATIVE_CURRENCY[chainId], MOCK_ZERO_DEC_TOKEN];
		...
		const v3MockPools = [USDC_DAI_LOW, USDC_DAI_MEDIUM, USDC_WETH_LOW, USDC_MOCK_LOW];
		...
		const v3MockSubgraphPools: V3SubgraphPool[] = _.map(v3MockPools, poolToV3SubgraphPool);
		...
		const v2MockSubgraphPools: V2SubgraphPool[] = _.map(v2MockPools, pairToV2SubgraphPool);
		...
		mockOnChainQuoteProvider.getQuotesManyExactOut.callsFake(
			async (amountOuts: CurrencyAmount[], routes: V3Route[], _providerConfig?: ProviderConfig) => {
		...
		mixedRouteMockGasModel.estimateGasCost.callsFake((r: MixedRouteWithValidQuote) => {
			return {
				gasEstimate: BigNumber.from(10000),
				gasCostInToken: CurrencyAmount.fromRawAmount(
					r.quoteToken,
