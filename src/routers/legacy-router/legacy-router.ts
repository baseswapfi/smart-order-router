...
import { SwapRouter, Trade } from '@baseswapfi/router-sdk';
import { ChainId, Currency, Token, TradeType } from '@baseswapfi/sdk-core';
import { FeeAmount, MethodParameters, Pool, Route } from '@baseswapfi/v3-sdk2';
...
import { DAI_BASE, ITokenProvider, USDbC_BASE } from '../../providers/token-provider';
...
import { BASES_TO_CHECK_TRADES_AGAINST, CUSTOM_BASES } from './bases';
...
export class LegacyRouter {
  ...
  public async routeExactIn(
    ...
    return {
      ...
      estimatedGasUsedUSD: CurrencyAmount.fromFractionalAmount(DAI_BASE!, 0, 1),
      ...
      return new V3RouteWithValidQuote({
        ...
        gasModel: {
          estimateGasCost: () => ({
            ...
            gasCostInUSD: CurrencyAmount.fromRawAmount(USDbC_BASE, 0),
  ...
  private async getAllPossiblePairings(tokenIn: Token, tokenOut: Token): Promise<[Token, Token, FeeAmount][]> {
    ...
    const additionalA: Token[] = [];
    const additionalB: Token[] = [];
